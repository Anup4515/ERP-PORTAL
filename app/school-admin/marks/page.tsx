                ...
                <tbody>
                  {students.map((s, idx) => (
                    <tr key={s.enrollment_id} className={`${idx % 2 === 0 ? "bg-white" : "bg-gray-50/40"} hover:bg-blue-50/20`}>
                      ...
                      <td className="bg-inherit px-2 py-2 text-gray-900 font-medium border-b border-r border-gray-100 whitespace-nowrap">{s.first_name} {s.last_name}</td>
                      <td className="bg-inherit px-2 py-2 text-gray-500 border-b border-r border-gray-100">{s.roll_number || "-"}</td>
                      {subjects.map((sub) => {
